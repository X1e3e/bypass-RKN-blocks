...
## 🔎 Требования к целевому домену (Dest/SNI) в технологии Reality

Выбор правильного домена-маски (SNI) критически важен для стабильности соединения. Если маскировочный домен выбран неверно, ТСПУ/DPI цензора быстро обнаружит аномалию и заблокирует ваш IP-адрес.

### 1. Поддержка TLS 1.3
Reality перехватывает и изменяет TLS-рукопожатие только при использовании протокола **TLS 1.3**. 
*   **Почему это важно:** В TLS 1.2 сертификат сервера передается в открытом виде (plaintext). Если клиент согласует TLS 1.2, ваш VPS-сервер будет вынужден отправить клиенту реальный сертификат целевого сайта (например, `apple.com`), приватного ключа от которого у вас нет. Это приведет к ошибке рукопожатия.
*   В TLS 1.3 сертификат передается в зашифрованном виде на этапе `EncryptedExtensions`. Reality-сервер может безопасно зашифровать сессию своими ключами, маскируя трафик под легитимный обмен ключами Diffie-Hellman.

### 2. Согласование ALPN (Application-Layer Protocol Negotiation)
При установлении соединения клиент отправляет список поддерживаемых протоколов (например, `h2` — HTTP/2, `http/1.1`). Reality-сервер должен прозрачно проксировать и согласовывать ALPN с оригинальным сайтом:
*   Если клиент запрашивает `h2`, а ваш Reality-сервер отвечает `http/1.1` (или наоборот), промежуточный DPI зафиксирует аномалию протокола.
*   Reality автоматически считывает ALPN из запроса клиента, делает быстрый запрос к реальному сайту-маске, получает его ALPN и возвращает клиенту корректный ответ.

### 3. Отсутствие HSTS (HTTP Strict Transport Security)
Некоторые крупные ресурсы жестко требуют использования HSTS и сверяют предзагруженные списки доменов (HSTS Preload List) в браузерах. Использование доменов с агрессивной политикой HSTS в качестве маски может приводить к ошибкам валидации сертификатов в браузерах пользователей при случайном обращении к IP-адресу вашего сервера.

### 4. Соответствие сетевых характеристик (Гео-локация и пинг)
Для систем DPI подозрительно выглядит ситуация, когда:
*   Пинг до IP-адреса вашего сервера (на котором настроен Reality) составляет 30 мс (сервер находится в Германии).
*   Пинг до оригинальных серверов домена-маски (например, крупного американского CDN) составляет 150 мс.
*   **Решение:** Старайтесь выбирать домены, физически расположенные на серверах той же страны или автономной системы (AS), где находится ваш VPS. Для хостинга в Германии маскируйте трафик под немецкие сайты или глобальные CDN (Cloudflare, Akamai), имеющие точки присутствия в Европе.

---

## ⚙️ Концептуальная структура клиентского JSON (Sing-box/Xray)

Клиентские приложения (Sing-box, Xray) настраиваются с помощью конфигурационного файла в формате JSON. Конфиг состоит из трех основных блоков:
...
### 1. `inbounds` (Входящие интерфейсы)
Этот блок описывает, как клиентское приложение принимает трафик от вашей операционной системы или других программ.
*   **SOCKS / HTTP:** Создает локальный прокси-сервер (например, `127.0.0.1:1080`), который прописывается в настройках браузера или Telegram.
*   **TUN (Tunnel):** Создает виртуальный сетевой адаптер в ОС. Весь трафик компьютера автоматически направляется в этот адаптер на уровне маршрутизации ядра, превращая клиент в полноценный VPN (не требует настройки прокси в приложениях).

### 2. `outbounds` (Исходящие интерфейсы)
Этот блок описывает, куда направлять отфильтрованный трафик. Обычно настраивается три выхода:
*   **VLESS/Reality:** Исходящий интерфейс, шифрующий трафик и отправляющий его на ваш зарубежный VPS.
*   **direct:** Прямой выход в сеть без шифрования (используется для обхода VPN при обращении к российским сайтам/банкам).
*   **block:** Сброс пакетов (для блокировки рекламы и трекеров).

### 3. `routing` (Маршрутизация)
Содержит правила (Rules), по которым клиент определяет, в какой `outbound` направить конкретный пакет.
*   Правила анализируют: IP-адрес назначения, доменное имя (через базы GeoIP и Geosite), порт или имя приложения.
*   Пример логики: если домен оканчивается на `.ru` или входит в категорию `category-gov-ru`, пакет идет в `direct`. Если домен совпадает со списком заблокированных, пакет перенаправляется в интерфейс `VLESS`.
...
## ❓ FAQ и Troubleshooting для VLESS-Reality

### 1. Ошибки TLS Handshake (Mismatch, Alert)
*   **Симптом:** При попытке подключения клиент выдает ошибку `TLS handshake failed` или `remote error: tls: alert(...)`.
*   **Диагностика:**
    1.  Проверьте, поддерживает ли маскировочный домен TLS 1.3 (используйте команду `openssl s_client -connect domain.com:443 -tls1_3`). Если домен не отвечает на TLS 1.3, замените его.
    2.  Проверьте соответствие публичного ключа (Public Key) на клиенте и приватного ключа на сервере.

### 2. Блокировка SNI по IP-адресу (Активное зондирование)
*   **Симптом:** Соединение работает несколько минут, после чего порт Reality полностью перестает отвечать с российских IP-адресов.
*   **Причина:** Цензор зафиксировал обращение к зарубежному IP по имени крупного бренда (например, `apple.com`) и выполнил активное зондирование: отправил запрос к вашему серверу без ключей Reality. Если сервер ответил некорректно (или вернул заглушку вместо оригинальной страницы Apple), IP отправляется во временный бан.
*   **Решение:** Тщательно проверьте параметр `Dest` на сервере. Reality должен гарантированно пересылать неавторизованные запросы на оригинальный сайт.

### 3. Несовместимость отпечатков браузеров (uTLS/ClientHello Fingerprinting)
*   **Симптом:** Соединение устанавливается с ПК, но не работает с мобильного телефона или Smart TV.
*   **Причина:** DPI цензора сравнивает структуру TLS ClientHello с типичным поведением клиента. Если в конфиге Reality указан отпечаток `chrome`, а TCP-стек мобильного телефона ведет себя как Android-устройство с другими параметрами (window size, MTU), соединение классифицируется как аномалия.
*   **Решение:** В клиентах настраивайте параметр **uTLS / Fingerprint** (например, выставляйте профиль `chrome` или `randomize`), чтобы сгладить различия сетевого уровня.
