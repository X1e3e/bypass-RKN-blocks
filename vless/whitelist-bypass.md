...
#### 🐝 Особенности Beeline CDN (Бесплатный безлимит)
...
#### 📱 Особенности Yandex Cloud CDN
При регистрации нового аккаунта выдается приветственный грант в 4000 ₽. Этого гранта хватает примерно на 5-6 ТБ трафика (около месяца бесплатной работы CDN для небольшого проекта). Однако в некоторых южных регионах (например, Краснодарский край) мобильные операторы местами блокируют или замедляют трафик к серверам Yandex Cloud CDN.
...
4.  **Устранение утечек DNS в клиенте Incy:**
    При работе с CDN-нодами в Incy могут происходить утечки DNS-запросов к заблокированным сайтам. Для исправления полностью удалите секцию `"dns"` из клиентской JSON-конфигурации Xray. Это принудительно направит весь DNS-трафик через защищенный CDN-прокси.
